...
python3 experiments.py evaluate configs/AdvIntersectionEnv/env_multi_agent_dest_good_agent.json \
                               configs/AdvIntersectionEnv/agents/DQNAgent/ego_attention.json \
                               --test --recover-from=out/MultiAgentAdvIntersectionEnv/DQNAgent/good_agent_ego_attention_20220820-205234_299323/checkpoint-best.tar \
                               --episodes=1000 --name-from-config --no-display 

# zero_sum
python3 experiments.py evaluate configs/AdvIntersectionEnv/env_multi_agent_dest_zero_sum.json \
                               configs/AdvIntersectionEnv/agents/DQNAgent/ego_attention.json \
				               --test --recover-from=out/MultiAgentAdvIntersectionEnv/DQNAgent/zero_sum_ego_attention_20220820-215352_301239/checkpoint-best.tar \
                                --episodes=1000 --name-from-config --no-display

# failmaker
python3 experiments.py evaluate configs/AdvIntersectionEnv/env_multi_agent_dest_failmaker.json \
                               configs/AdvIntersectionEnv/agents/DQNAgent/ego_attention.json \
				                --test --recover-from=out/MultiAgentAdvIntersectionEnv/DQNAgent/failmaker_ego_attention_20220820-225607_303165/checkpoint-best.tar \
                                --episodes=1000 --name-from-config --no-display
